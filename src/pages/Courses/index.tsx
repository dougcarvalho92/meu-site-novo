...
import { LoadMore, PageCounter, SearchBar } from "./styles";
...
const Course: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function FilterCourses() {
      await api.get(`/courses/?search=${filteredInput}`).then((result) => {
  ...
  async function handleLoadMore() {
    ...
    if (selectedPage <= pagesCount) {
      ...
      const newPage = await api
        .get(`/courses/?search=${filteredInput}&page=${selectedPage + 1}`)
        .then((result) => result.data.docs);
